...
def get_data():
    data_train = pd.read_csv('drgsreviews/data/train_test_Keras.txt', sep='\t', encoding='Latin-1', error_bad_lines=False)     
    user_file = pd.read_csv('drgsreviews/data/userList.txt', sep='\t', encoding='Latin-1', error_bad_lines=False)
    user_file.user= user_file.user.astype(str)
    drug_file = pd.read_csv('drgsreviews/data/drugList.txt', sep='\t', encoding='Latin-1', error_bad_lines=False)
...
def execute_model(data_dic):
    ...
    # building Hierachical Attention network + UPA
        ...
        model.load_weights('drgsreviews/data/trained_weights.h5')
        ...
        res = np.argmax(ylabel)+1, np.argmax(ypred)+1
